...
public:
	...
	ObstacleDetection(int, int, int, int, int, int, int, int, int, int, float, float);
	~ObstacleDetection();
	void detectObs(cv::Mat *, cv::Mat *, cv::Mat *, cv::Mat *, cv::Mat *, cv::Mat *, cv::Mat *, double, bool);
...
private:
	...
	void getFlowVectors(std::vector<cv::Point2f> *, std::vector<cv::Point2f> *, std::vector<cv::Point2f> *, std::vector<cv::Point2f> *, float);
	...
	cv::Point2f findFOEDivergence(std::vector<cv::Point2f> , cv::Mat , int, int);
	cv::Point2f findFOEDivergence(std::vector<cv::Point2f> *, std::vector<cv::Point2f> *, float , int , int );
	void timeToContact(std::vector<cv::Point2f> *, cv::Mat *, cv::Point2f, cv::Mat *, cv::Mat **, cv::Mat **);
	void formDepthImg(cv::Mat ***, std::vector<cv::Point2f> *, std::vector<float> *, float, float, int, int);
	void formDepthMap(cv::Mat ***, std::vector<cv::Point2f> *, std::vector<float> *, float, float, int, int);
